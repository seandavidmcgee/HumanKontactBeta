...
extension RecentsSubViewController: UITableViewDataSource
{
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return Int(RecentPeople.recents.count)
    }
    
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell
    {
        var hkPerson = hkRecentSorted[Int(indexPath.row)] as HKPerson
            ...
            if hkPerson.phoneNumbers.count > 1 {
                for phone in hkPerson.phoneNumbers {
                    ...
                    if let profileLabel = profilePhoneLabel(phoneStrings) as String! {
                        ...
                        recentsPhonesArray.append(localPhone)
                    } else {
        ...
        if let hkEmail = hkPerson.emails.first as HKEmail! {
            if hkPerson.emails.count > 1 {
                for email in hkPerson.emails {
                    ...
                    recentsEmailsArray.append(localEmail)
...
extension RecentsSubViewController: UISearchResultsUpdating
{
    func updateSearchResultsForSearchController(searchController: UISearchController)
    {
        ...
        if searchString == "" {
            queriedSearch = recentRealm.objects(HKPerson).filter("firstName BEGINSWITH[c] '\(keyEntry)' OR lastName BEGINSWITH[c] '\(keyEntry)'")
        } else {
            queriedSearch = recentRealm.objects(HKPerson).filter("fullName CONTAINS[c] '\(keyEntry)' OR fullName CONTAINS[c] '\(searchString)' OR firstName CONTAINS[c] '\(searchString)' OR lastName CONTAINS[c] '\(searchString)'")
        }
        RecentPeople.recents = queriedSearch
