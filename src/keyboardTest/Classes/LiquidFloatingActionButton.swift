...
public class LiquidFloatingActionButton : UIView {
    ...
    public var clearColor: UIColor = UIColor.clearColor()
    private let internalRadiusRatio: CGFloat = 20.0 / 42.0
    ...
    private func drawCircle() {
        ...
        if touching && responsible {
            ...
            self.circleLayer.backgroundColor = self.clearColor.CGColor
        }
    }
...
class CircleLiquidBaseView : ActionBarBaseView {
    ...
    var baseLiquidOpen: LiquittableCircle?
    ...
    override func setup(actionButton: LiquidFloatingActionButton) {
        ...
        baseLiquid = LiquittableCircle(center: self.center.minus(self.frame.origin), radius: radius, color: self.color)
        baseLiquidOpen = LiquittableCircle(center: self.center.minus(self.frame.origin), radius: radius, color: actionButton.color)
        baseLiquid?.clipsToBounds = false
        baseLiquid?.layer.masksToBounds = false
        baseLiquidOpen?.clipsToBounds = false
        baseLiquidOpen?.layer.masksToBounds = false
        baseLiquidOpen?.hidden = true
        
        clipsToBounds = false
        layer.masksToBounds = false
        addSubview(baseLiquid!)
        addSubview(baseLiquidOpen!)
    }
    ...
    func update(delay: CGFloat, duration: CGFloat, f: (LiquidFloatingCell, Int, CGFloat) -> ()) {
        ...
        if opening {
            engine?.draw(baseLiquidOpen!)
            bigEngine?.draw(baseLiquidOpen!)
            baseLiquidOpen?.hidden = false
            baseLiquid?.hidden = true
        } else {
            engine?.draw(baseLiquid!)
            bigEngine?.draw(baseLiquid!)
            baseLiquidOpen?.hidden = true
            baseLiquid?.hidden = false
        }
    }
    ...
    func updateClose() {
        update(0, duration: closeDuration) { cell, i, ratio in
            let distance = (cell.frame.height * 0.5 + CGFloat(i + 1) * cell.frame.height * 1.5) * (0.9935 - ratio)
            cell.center = self.center.plus(self.differencePoint(distance))
...
public class LiquidFloatingCell : LiquittableCircle {
    ...
    init(center: CGPoint, radius: CGFloat, color: UIColor, icon: UIImage, label: String) {
        self.originalColor = color
        super.init(center: center, radius: radius, color: color)
        setup(icon, text: label)
    }
    ...
    func setup(image: UIImage, tintColor: UIColor = UIColor.whiteColor(), text: String) {
        imageView.image = image.imageWithRenderingMode(UIImageRenderingMode.AlwaysTemplate)
        imageView.tintColor = tintColor
        imageLabel.textColor = tintColor
        imageLabel.textAlignment = .Center
        imageLabel.font = UIFont(name: "AvenirNext-Regular", size: 12)
    ...
    private func resizeSubviews() {
        ...
        let labelSize = CGSize(width: frame.width, height: frame.height * 0.3)
        ...
        imageLabel.frame = CGRect(x: -20, y: frame.height + 5, width: frame.width + 40, height: labelSize.height)
    }
