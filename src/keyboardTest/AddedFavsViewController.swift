...
struct FavPeople {
    static var favorites = favRealm.objects(HKPerson)
}

let favRealm = ABManager.favoriteRealm()
...
class AddedFavsViewController: UITableViewController, UITextFieldDelegate {
    ...
    var hkFavSorted: Results<HKPerson>!
    ...
    override func viewDidLoad() {
        ...
        if let hkFavSorted_ = FavPeople.favorites.sorted("created", ascending: false) as Results<HKPerson>! {
            hkFavSorted = hkFavSorted_
        }
    ...
    internal func fetchFavorites() {
        manager.favoriteRecords({ [weak self]() -> () in
            dispatch_async(dispatch_get_main_queue(), { () -> Void in
                favPeople.removeAll(keepCapacity: false)
                self?.tableView.reloadData()
            })
